...
static gboolean
setup_png_transformations (png_structp png_read_ptr, png_infop png_info_ptr, GError **error, png_uint_32 *width_p, png_uint_32 *height_p, int *color_type_p)
{
...
{
  cairo_surface_t *volatile pixbuf = NULL;
  gint rowstride;
  png_structp png_ptr;
  png_infop info_ptr;
  png_textp text_ptr;
  ...
  // TODO pixbuf = gdk_pixbuf_new (GDK_COLORSPACE_RGB, ctype & PNG_COLOR_MASK_ALPHA, 8, w, h);
  pixbuf = cairo_image_surface_create (CAIRO_FORMAT_ARGB32, w, h); // TODO
  ...
  // rowstride = gdk_pixbuf_get_rowstride (pixbuf);//TODO
  rowstride = cairo_image_surface_get_stride (pixbuf) * 4; // TODO

  // TODO gdk_pixbuf_fill (pixbuf, DEFAULT_FILL_COLOR);

  rows = g_new (png_bytep, h);

  for (i = 0, ptr = cairo_image_surface_get_data (pixbuf); i < h; i++, ptr = (guchar *) ptr + rowstride)
    rows[i] = ptr;
...
gdk_pixbuf__png_image_stop_load (gpointer context, GError **error)
{
  ...
  if (lc->pixbuf)
    ...
    {
      g_set_error_literal (error, GPAINT_FORMAT_ERROR,
                           GPAINT_FORMAT_ERROR_CORRUPT_IMAGE,
  ...
  if (lc->fatal_error_occurred)
  ...
  else
    {
      if (lc->first_row_seen_in_chunk >= 0)
        {
          gint width = cairo_surface_get_width (lc->pixbuf);
                   ...
                   png_infop png_info_ptr)
{
  ...
  lc->pixbuf = cairo_image_surface_create (CAIRO_FORMAT_ARGB32, width, height);
  ...
  // TODO gdk_pixbuf_fill (lc->pixbuf, DEFAULT_FILL_COLOR);
                  ...
                  int pass_num)
{
  ...
  if (row_num >= cairo_surface_get_height (lc->pixbuf))
    {
      lc->fatal_error_occurred = TRUE;
      g_set_error_literal (lc->error,
                           GPAINT_FORMAT_ERROR,
               ...
               gpointer user_data)
{
          ...
          if (len < 1 || len > 79)
            {
               ...
               * PNG text key
               */
              g_set_error (error, GPAINT_FORMAT_ERROR,
                           GPAINT_FORMAT_ERROR_BAD_OPTION,
  ...
  // TODO bpc = gdk_pixbuf_get_bits_per_sample (pixbuf);
  bpc = 32; // TODO
  w = cairo_image_surface_get_width (pixbuf);
  h = cairo_image_surface_get_height (pixbuf);
  rowstride = cairo_image_surface_get_stride (pixbuf);
  has_alpha = true; // TODO gdk_pixbuf_get_has_alpha (pixbuf);
  pixels = cairo_image_surface_get_data (pixbuf);
