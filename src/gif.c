...
static ColorMapObject *build_websafe_palette(void) {
    int numColors = 216;
...
static int pixel_to_index(uint32_t pixel) {
    int r = (pixel >> 16) & 0xff;
    int g = (pixel >> 8) & 0xff;
    int b = pixel & 0xff;
    int ri = r / 51;
...
int gif_save_anim(const char *filename, cairo_surface_t **frames, int nframes, int delay) {
    if (nframes <= 0)
        return -1;
    int error;
