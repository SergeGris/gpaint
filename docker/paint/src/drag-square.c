...
enum { GPAINT_DRAG_TRACKER_SIZE = 8 };
...
drag_update (GtkGestureDrag *gesture, gdouble x, gdouble y, gpointer user_data)
{
  ...
  char buffer[64];
  gint tx = (gint) (x / state->zoom_level);
  gint ty = (gint) (y / state->zoom_level);

  if (self->dirx != 0)
    {
      snprintf (buffer, sizeof (buffer), "W: %d (%c%d)",
                cairo_image_surface_get_width (state->main_surface) + tx, "+-"[tx < 0], abs(tx));
