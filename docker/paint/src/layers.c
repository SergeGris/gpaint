...
MYLayerItem* my_layer_item_new(cairo_surface_t *surface) {
    MYLayerItem *item = g_object_new(MY_TYPE_LAYER_ITEM, NULL);
    item->surface = surface;
    return item;
}

cairo_surface_t* my_layer_item_get_surface(MYLayerItem *item) {
    return item->surface;
}
...
static void my_layer_manager_update_selection(MYLayerManager *manager) {
...
static void on_layer_item_toggled(GtkToggleButton *button, gpointer user_data) {
...
static void on_move_up_button_clicked(GtkButton *button, gpointer user_data) {
    MYLayerManager *manager = MY_LAYER_MANAGER(user_data);
    for (GList *l = manager->layer_items; l; l = l->next) {
        MYLayerItem *item = l->data;
...
static void on_move_down_button_clicked(GtkButton *button, gpointer user_data) {
    MYLayerManager *manager = MY_LAYER_MANAGER(user_data);
    for (GList *l = manager->layer_items; l; l = l->next) {
        MYLayerItem *item = l->data;
...
static void my_layer_manager_init(MYLayerManager *self) {
...
static void my_layer_manager_class_init(MYLayerManagerClass *klass) {
    // Nothing special needed.
}

GtkWidget* my_layer_manager_new(void) {
...
cairo_surface_t* my_layer_manager_get_current_surface(MYLayerManager *manager) {
    for (GList *l = manager->layer_items; l; l = l->next) {
        MYLayerItem *item = l->data;
        if (item->selected) {
