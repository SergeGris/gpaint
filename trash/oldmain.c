...
update_cursor (AppState *state)
{
  ...
  if (state->tool->draw_cursor_handler != NULL)
    ...
    {
      /* g_autoptr (GdkTexture) texture = gdk_texture_new_from_filename (state->tool->icon, NULL); */

      /* if (!texture) */
      /*   { */
      /*     g_warning ("Failed to create GdkTexture from filename: %s", state->tool->icon); */
      /*     return; */
      /*   } */
